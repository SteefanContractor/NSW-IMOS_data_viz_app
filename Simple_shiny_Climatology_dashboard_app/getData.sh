...
###########################################################################
# Download ocean colour (chlorophyll-a) data
###########################################################################

if [ ! -d data/CHL_OC3 ]; then mkdir data/CHL_OC3; fi
cd data/CHL_OC3
# a file to distinguish old from new files
touch lastwatch
...
# Get the latest two available months
wget -q -O - http://thredds.aodn.org.au/thredds/catalog/IMOS/SRS/OC/gridded/aqua/P1D/2019/catalog.html | grep -oP  "(?<=href=')[0-9][0-9](?=/catalog.html')" | sort -nr >> months.txt
thismon=`head -n 1 months.txt`
lastmon=`head -n 2 months.txt | tail -n 1`

# Get the name of latest file of ocean colour data from this month
wget -q -O - http://thredds.aodn.org.au/thredds/catalog/IMOS/SRS/OC/gridded/aqua/P1D/2019/"$thismon"/catalog.html | grep -oP  "(?<=IMOS/SRS/OC/gridded/aqua/P1D/2019/"$thismon"/).*aust.chl_oc3.nc(?='><tt>)" | tail -n 7 >> fnames.txt

# Download files in fnames.txt
for f in `cat fnames.txt`; do
	wget -q -nc -A.nc http://thredds.aodn.org.au/thredds/fileServer/IMOS/SRS/OC/gridded/aqua/P1D/2019/"$thismon"/"$f"
done

# If there aren't 7 files in the current month then get the remaining files from last month
totfiles=`cat fnames.txt | wc -l`
if [ $totfiles -lt 7 ]; then
	remainfiles=$(( 7 - $totfiles ))		
	wget -q -O - http://thredds.aodn.org.au/thredds/catalog/IMOS/SRS/OC/gridded/aqua/P1D/2019/"$lastmon"/catalog.html | grep -oP  "(?<=IMOS/SRS/OC/gridded/aqua/P1D/2019/"$lastmon"/).*aust.chl_oc3.nc(?='><tt>)" | tail -n "$remainfiles" > fnames_lastmon.txt
	for f in `cat fnames_lastmon.txt`; do
        	wget -q -nc -A.nc http://thredds.aodn.org.au/thredds/fileServer/IMOS/SRS/OC/gridded/aqua/P1D/2019/"$lastmon"/"$f"
	done
	cat fnames_lastmon.txt >> fnames.txt
fi
