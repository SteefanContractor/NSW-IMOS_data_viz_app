...
load(paste0(basePath, "CHL_OC3/CHL_OC3.RData"))
      ...
      ")),
               tabItems(
                 tabItem("Home",
                         ...
                         #           color = "blue", fill = T)
                         # ),
                         wellPanel(selectInput(inputId = "SSTtype",
                                               label = "Base layer",
                                               choices = c("SST", "Warm SSTs", "Cold SSTs", "Ocean Colour (Chl-a)"))),
                         htmlOutput("frame"),
                         # HTML("<iframe src='figures/homemap_ggplotly_SSTonly.html' width='100%' height='500'></iframe>"),
...
server <- function(input, output){
  ...
  output$frame <- renderUI({
    ssttype <- input$SSTtype
    if (ssttype == "SST") {
      homemap <- tags$iframe(src="figures/homemap_ggplotly_SSTonly.html", width="100%", height="500")
    } else if (ssttype == "Warm SSTs") {
      homemap <- tags$iframe(src="figures/homemap_ggplotly_warmSSTs.html", width="100%", height="500")
    } else if (ssttype == "Cold SSTs") {
      homemap <- tags$iframe(src="figures/homemap_ggplotly_coldSSTs.html", width="100%", height="500")
    } else {
      homemap <- tags$iframe(src="figures/homemap_ggplotly_CHL-OC3.html", width="100%", height="500")
    }
    homemap
  })
