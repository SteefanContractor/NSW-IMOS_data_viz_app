...
if (local) {
  basePath <- #"~/ownCloud/Working_Directory/Postdoc-SchoolOfMathsStats/Data/Climatologies_from_Michael/climatology/data/"
...
ui <- 
  ...
  navbarPage(theme = shinythemes::shinytheme("flatly"),
             "Port Hacking PH100/NRSPHB Mooring",
             tabPanel("Home",
               fluidRow(
                 infoBox("Temperature", 20, icon = icon("calendar"), fill = T)
               )
             ),
             ...
             tabPanel("Marine Temperature Anomalies",
                      sidebarLayout(
